...
public class TagRegistration extends BlockTagsProvider {
    ...
    private BeerBuilder valueLookupBuilder(TagKey<Block> tag) {
        BlockItemTagAppender<Block> builder = builder(tag);
        return new BeerBuilder(builder);
    }

    @SuppressWarnings({"UnusedReturnValue", "SameParameterValue"})
    private record BeerBuilder(BlockItemTagAppender<Block> builder) {

        private BeerBuilder add(Block... blocks) {
            for (Block block : blocks) {
                Optional<ResourceKey<Block>> resourceKey = BuiltInRegistries.BLOCK.getResourceKey(block);
                this.builder.add(resourceKey.orElseThrow());
            }
            return this;
        }

        private BeerBuilder add(BlockItemId... ids) {
